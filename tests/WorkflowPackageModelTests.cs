...
public class ConsultGenerationProvenanceTests
{
    private static ConsultGenerationRequest CreateRequest() =>
        new(
            "Draft consult text.",
            new[]
            {
                new ConsultGenerationSectionRequest("hpi", "History of Present Illness", "Standard A"),
                new ConsultGenerationSectionRequest("pmh", "Past Medical History", "Standard B")
            });

    [Fact]
    public void ComputeEffectiveInputHash_IsStableForEqualContent()
    {
        var first = ConsultGenerationProvenance.ComputeEffectiveInputHash(CreateRequest());
        var second = ConsultGenerationProvenance.ComputeEffectiveInputHash(CreateRequest());
    ...
    [Fact]
    public void ComputeEffectiveInputHash_ChangesWhenContentChanges()
    {
        var baseline = ConsultGenerationProvenance.ComputeEffectiveInputHash(CreateRequest());

        var draftChanged = ConsultGenerationProvenance.ComputeEffectiveInputHash(
            CreateRequest() with { ConsultDraft = "Different draft." });
        var standardChanged = ConsultGenerationProvenance.ComputeEffectiveInputHash(
            new ConsultGenerationRequest(
                "Draft consult text.",
                new[] { new ConsultGenerationSectionRequest("hpi", "History of Present Illness", "Standard CHANGED") }));

        Assert.NotEqual(baseline, draftChanged);
        Assert.NotEqual(baseline, standardChanged);
    }

    [Fact]
    public void ComputeEffectiveInputHash_IgnoresWorkflowPackageField()
    {
        // The package is recorded separately in provenance; the input hash covers the
        // clinical content only.
        var withoutPackage = ConsultGenerationProvenance.ComputeEffectiveInputHash(CreateRequest());
        var withPackage = ConsultGenerationProvenance.ComputeEffectiveInputHash(
            CreateRequest() with { WorkflowPackage = "general@v2026.07.1" });

        Assert.Equal(withoutPackage, withPackage);
    }
}
